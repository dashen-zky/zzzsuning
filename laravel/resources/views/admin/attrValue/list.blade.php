...
<div class="mws-panel grid_8">
  <div class="mws-panel-header">
    <span style="text-shadow: 1px 1px 1px rgba(0, 0, 0, 0)"> <i class="icon-table"></i>属性值列表
     <a href="/admin/attrValue/add" style="float:right;clear:both"><i class="icon-plus"></i>添加销售属性值</a></span>
  </div>
  <div class="mws-panel-body no-padding">
    ...
    <form action="/admin/attr/index">
      ...
      <table class="mws-datatable-fn mws-table dataTable" id="DataTables_Table_1" aria-describedby="DataTables_Table_1_info" >
        <thead>
          <tr role="row">
            <th class="sorting_asc" role="columnheader" tabindex="0" aria-controls="DataTables_Table_1" rowspan="1" colspan="1" aria-sort="ascending" aria-label="Rendering engine: activate to sort column descending" style="width: 30px;">属性值ID</th>
            <th class="sorting" role="columnheader" tabindex="0" aria-controls="DataTables_Table_1" rowspan="1" colspan="1" aria-label="Browser: activate to sort column ascending" style="width: 70px;">属性值名称</th>
            <th class="sorting" role="columnheader" tabindex="0" aria-controls="DataTables_Table_1" rowspan="1" colspan="1" aria-label="Engine version: activate to sort column ascending" style="width: 40px;">属性值所属属性</th>
            <th class="sorting" role="columnheader" tabindex="0" aria-controls="DataTables_Table_1" rowspan="1" colspan="1" aria-label="Engine version: activate to sort column ascending" style="width: 40px;">属性值状态</th>
        ...
        <tbody role="alert" aria-live="polite" aria-relevant="all" >
          @foreach($attrValues as $k=>$v)
         	<tr class="@if($k%2 == 0) even @else odd @endif" style="text-align:center">
              ...
              <td class=" ">{{$v->name}}</td>
              <td class=" ">{{$v->attr->name}}</td>
	            <td class=" "><input bid="{{$v->id}}" class="ibutton" type="checkbox" @if($v->status==1) checked="checked" @endif ></td>
	            <td class=" ">
	            	<span class="btn-group" >
	                    <a href="/admin/attrValue/edit?id={{$v->id}}" class="btn btn-small"><i class="icon-pencil"></i>  编辑</a>
                      <a href="/admin/attrValue/delete?id={{$v->id}}" class="btn btn-small" name="delete"><i class="icon-trash"></i>  删除</a>
	                </span>
              </td>
            </tr>
          @endforeach
        ...
        <div style="padding-left:0px;margin-right:0px;" class="dataTables_paginate paging_full_numbers" id="DataTables_Table_1_paginate">
          <div id="pages">
            {!! $attrValues->appends($request->all())->render() !!}      
          </div>
        </div>
        <style type="text/css">
            #pages{
...
@section('js')
    ...
    $(function(){
        $(".ibutton-container").click(function(){
            ...
            //发送ajax
            $.get("/admin/attrValue/ajax-update",{id:id,status:status},function(data){
                if(data == '1'){
                  alert("更改成功");
                }else{
                  alert("更改失败");
