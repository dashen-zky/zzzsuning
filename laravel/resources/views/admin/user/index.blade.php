        ...
        <table class="mws-table mws-datatable dataTable" id="DataTables_Table_0" aria-describedby="DataTables_Table_0_info">
          <thead>
            <tr role="row">
              ...
              <th class="sorting" role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" style="width: 173px;" aria-label="Platform(s): activate to sort column ascending">邮箱地址</th>
              <th class="sorting" role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" style="width: 173px;" aria-label="Platform(s): activate to sort column ascending">最后登录的时间</th>
              <th class="sorting" role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" style="width: 116px;" aria-label="Engine version: activate to sort column ascending">用户激活状态</th>
          ...
          @foreach($info as $k=>$v)
            <tr class="@if($k%2==0) even @else odd @endif">
              ...
              <td class=" ">{!!date("Y-m-d H:i:s",$v->lasttime)!!}</td>
                <td class=" "><input cid="{{$v->id}}" class="ibutton" type="checkbox" @if($v->auth) checked="checked" @endif></td>
              <td class=" ">
                <span class="btn-group">
                  <a class="btn btn-small" href="/admin/user/detail?id={{$v->id}}">
                    <i class="icon-search"></i>
                  </a>
                  <a class="btn btn-small" href="/admin/user/edit?id={{$v->id}}">
                    <i class="icon-pencil"></i>
                  </a>
                  <a class="btn btn-small" href="/admin/user/delete?id={{$v->id}}">
...
@section('js')
  <script src="/admins/js/libs/jquery-1.8.3.min.js"></script>
    ...
    $('.ibutton-container').click(function(){
      //获取状态
      var auth = $(this).find('input')[0].checked ? 1 : 0;
      var id = $(this).find('input').attr('cid');
      //发送
      $.get('/admin/user/ajax-update', {auth: auth, id: id}, function(data){
        if(data == '1'){
          alert('更新成功');
        }else{
          alert('更新失败');
