...
@section('content')
    <div class="mws-panel grid_4">
        ...
        <div class="mws-panel-body no-padding">
            <form class="mws-form" action="/admin/specValue/insert" method="post" enctype="multipart/form-data">
            @if (count($errors) > 0)
  ...
  <div class="mws-panel-header">
    <span style="text-shadow: 1px 1px 1px rgba(0, 0, 0, 0)">
      <i class="icon-table"></i>规格值列表</span>
  </div>
  <div class="mws-panel-body no-padding">
    <div id="DataTables_Table_1_wrapper" class="dataTables_wrapper" role="grid">
    <form action="/admin/specValue/add">
      ...
      <table class="mws-datatable-fn mws-table dataTable" id="DataTables_Table_1" aria-describedby="DataTables_Table_1_info" >
        <thead>
          <tr role="row">
            <th class="sorting_asc" role="columnheader" tabindex="0" aria-controls="DataTables_Table_1" rowspan="1" colspan="1" aria-sort="ascending" aria-label="Rendering engine: activate to sort column descending" style="width: 30px;">规格ID</th>
            <th class="sorting" role="columnheader" tabindex="0" aria-controls="DataTables_Table_1" rowspan="1" colspan="1" aria-label="Browser: activate to sort column ascending" style="width: 70px;">规格名称</th>
            <th class="sorting" role="columnheader" tabindex="0" aria-controls="DataTables_Table_1" rowspan="1" colspan="1" aria-label="Engine version: activate to sort column ascending" style="width: 40px;">规格所类型</th>
            <th class="sorting" role="columnheader" tabindex="0" aria-controls="DataTables_Table_1" rowspan="1" colspan="1" aria-label="Engine version: activate to sort column ascending" style="width: 40px;">规格状态</th>
          ...
          <tr class="@if($k%2 == 0) even @else odd @endif" style="text-align:center">
              ...
              <td class=" ">
                <span class="btn-group" >
                      <a href="/admin/specValue/edit?id={{$v->id}}" class="btn btn-small"><i class="icon-pencil"></i>  编辑</a>
                      <a href="/admin/specValue/delete?id={{$v->id}}" class="btn btn-small" name="delete"><i class="icon-trash"></i>  删除</a>
                  </span>
              </td>
            </tr>
          @endforeach
        ...
        <div style="padding-left:0px;margin-right:0px;" class="dataTables_paginate paging_full_numbers" id="DataTables_Table_1_paginate">
          <div id="pages">
            {!! $specValues->appends($request->all())->render() !!}      
          </div>
        </div>
        <style type="text/css">
            #pages{
...
@section('js')
    ...
    $(function(){
        $(".ibutton-container").click(function(){
            ...
            //发送ajax
            $.get("/admin/specValue/ajax-update",{id:id,status:status},function(data){
                if(data == '1'){
                  alert("更改成功");
                }else{
                  alert("更改失败");
