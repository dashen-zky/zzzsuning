...
<div class="mws-panel grid_8">
  <div class="mws-panel-header">
    <span style="text-shadow: 1px 1px 1px rgba(0, 0, 0, 0)">
      <i class="icon-table"></i>属性列表</span>
  </div>
  <div class="mws-panel-body no-padding">
    ...
    <form action="/admin/attr/index">
      ...
      <table class="mws-datatable-fn mws-table dataTable" id="DataTables_Table_1" aria-describedby="DataTables_Table_1_info" >
        <thead>
          <tr role="row">
            <th class="sorting_asc" role="columnheader" tabindex="0" aria-controls="DataTables_Table_1" rowspan="1" colspan="1" aria-sort="ascending" aria-label="Rendering engine: activate to sort column descending" style="width: 30px;">属性ID</th>
            <th class="sorting" role="columnheader" tabindex="0" aria-controls="DataTables_Table_1" rowspan="1" colspan="1" aria-label="Browser: activate to sort column ascending" style="width: 70px;">属性名称</th>
            <th class="sorting" role="columnheader" tabindex="0" aria-controls="DataTables_Table_1" rowspan="1" colspan="1" aria-label="Engine version: activate to sort column ascending" style="width: 40px;">属性所类型</th>
            <th class="sorting" role="columnheader" tabindex="0" aria-controls="DataTables_Table_1" rowspan="1" colspan="1" aria-label="Engine version: activate to sort column ascending" style="width: 40px;">属性状态</th>
              ...
              <td class=" ">{{$v->type->name}}</td>
	            ...
	            <td class=" ">
	            	<span class="btn-group" >
	                    <a href="/admin/attr/edit?id={{$v->id}}" class="btn btn-small"><i class="icon-pencil"></i>  编辑</a>
                      <a href="/admin/attr/delete?id={{$v->id}}" class="btn btn-small" name="delete"><i class="icon-trash"></i>  删除</a>
	                </span>
              </td>
            </tr>
          @endforeach
...
@section('js')
    ...
    $(function(){
        $(".ibutton-container").click(function(){
            ...
            //发送ajax
            $.get("/admin/attr/ajax-update",{id:id,status:status},function(data){
                if(data == '1'){
                  alert("更改成功");
                }else{
                  alert("更改失败");
